            ...
            return (
              <div key={label} className="flex-1 text-center">
                <div className={`h-1.5 rounded-full transition-all duration-500 ${
                  idx <= stepIdx ? 'bg-eco-green' : 'bg-gray-200'
                }`} />
                <span className={`text-[10px] mt-1 block ${
                  idx <= stepIdx ? 'text-eco-green font-semibold' : 'text-gray-400'
                }`}>{label}</span>
              </div>
            )
          })}
                ...
                {loadedPhoto ? (
                  <div className="h-24 bg-gradient-to-br from-eco-green/10 to-coffee-brown/10 rounded-xl flex items-center justify-center">
                    <div className="flex items-center gap-2">
                      <ImageIcon className="w-5 h-5 text-eco-green" />
                      <span className="text-sm text-eco-green font-medium">계량판 사진 완료</span>
                    </div>
                  </div>
                ) : (
                  <motion.button
                    whileTap={{ scale: 0.97 }}
                    onClick={() => setLoadedPhoto(true)}
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            ...
            {emptyWeight && (
              ...
              >
                ...
                <p className="text-3xl font-bold text-eco-green mt-1">
                  {netWeight.toLocaleString()} <span className="text-lg">kg</span>
                </p>
                <p className="text-xs text-eco-green/70 mt-1">
                  예상 정산액: {(netWeight * 80).toLocaleString()}원
                </p>
              </motion.div>
            )}
            <motion.button
